...
class CommandStructure {
    ...
    async searchingMessage(message, timeoutDuration) {
        const statusMessage = await this.embed.create(message.channel.id, {
            title: "Searching...",
        });
        ...
        statusMessage.timeout = setTimeout(() => {
            this.embed.error(
                statusMessage,
                "Bot took too long to respond. Try again later."
            );

            this.client.log.error(
                `${message.command} by ${message.author.id} timed out.`
            );
    ...
    APIOptions(options, custom) {
        return {
            ...custom,
            language: options.apiLanguage,
            region: options.apiRegion,
        };
    }
    ...
    resultStructure(results, page, size) {
        ...
        return {
            ...
            results: pages[page].map((item, index) => ({
                ...item,
                index: (page + 1) * size - size + index + 1,
            })),
        };
    }
    ...
    runtime(value) {
        return value
            ? Array.isArray(value)
                ? value.map((r) => this.duration(r)).join(", ")
                : this.duration(value)
            : this.check(value);
    }
     ...
     */
    description(value) {
        return value && value.length > 2048
            ? value.substr(0, 2045) + "..."
            : this.check(value);
    }
