        ...
        this.embed.edit(statusMessage, {
            ...
            fields: flags.more
                ? this.fields([
                    ...
                    {
                        name: `Production Countr${this.plural(
                            response.production_countries,
                            true
                        )}`,
                        value: this.list(
                            response.production_countries.map((c) => c.name)
                        ),
                        inline: false,
                    },
                    {
                        name: `Production Compan${this.plural(
                            response.production_companies,
                            true
                        )}`,
                        value: this.list(
                            response.production_companies.map((c) => c.name)
                        ),
                        inline: false,
                    },
                    {
                ...
                : this.fields([
                    ...
                    response.vote_average
                        ? {
                            name: "⭐ — Vote Average",
                            value:
                                `**${this.check(response.vote_average)}** ` +
                    ...
                    response.status === "Released"
                        ...
                        : {
                            ...
                            name: "🗣️ — Language",
                            value: this.list(
                                response.spoken_languages.slice(0, 1).map((l) => l.name)
                            ),
                        },
                ]),
