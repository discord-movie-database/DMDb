...
class HelpCommand extends CommandStructure {
  ...
  constructor(client) {
    super(client, {
      description:
        "Get a list of commands or information for a specific command.",
      usage: "[Command Name]",
      flags: ["page"],
      developerOnly: false,
      hideInHelp: true,
  ...
  formatFlags(flags) {
    return flags
      ? flags
          .map((flag) => {
            return `[${
              this.flags.flagOptions[flag].argRequired
                ? `--${flag} <#>`
                : `--${flag}`
            }]`;
          })
  ...
  commandInfo(message, commandName) {
    ...
    this.embed.create(message.channel.id, {
      ...
      fields: [
        ...
        {
          name: "Flags",
          value: this.formatFlags(command.meta.flags) || "N/A",
          inline: false,
        },
        {
          name: "Hide in Help",
          value: this.yesno(command.meta.hideInHelp),
          inline: true,
        },
        {
          name: "Developer Only",
          value: this.yesno(command.meta.developerOnly),
          inline: true,
        },
      ],
    });
  ...
  commandList(message, guildSettings, page) {
    ...
    const embed = {
      ...
      description:
        `> Use **\`${prefix}help [Page Number]\`** for more commands.\n` +
    ...
    for (let i = 0; i < pageCommands.length; i++) {
      ...
      embed.fields.push({
        name:
          `${prefix}${commandName} ${commandMeta.usage || ""} ` +
  ...
  async executeCommand(message, commandArguments, guildSettings) {
    ...
    if (message.content.length > 0) {
      // Flag page.
      if (flags.page)
        return this.commandList(message, guildSettings, flags.page);
      // Page.
