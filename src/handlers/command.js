...
class CommandHandler extends HandlerStructure {
    ...
    checkAlias(commandName) {
        ...
        for (var i = 0; i < commandList.length; i++) {
            var aliases = this.commands[commandList[i]].meta.aliases;
    ...
    async onMessageEvent(message) {
        ...
        if (guildSettings.disabledCommands.indexOf(commandName) > -1) {
            return guildSettings.commandDisabledMessage
                ? this.client.util
                    .getUtil("embed")
                    .error(message.channel.id, "Command disabled.")
                : null;
        }
        ...
        try {
            ...
            this.client.log.info(
                `u${user.id} executed ${commandName} in ` +
            ...
            this.client.util
                .getUtil("embed")
                .error("Error executing command. Try again later.");
        }
    }
}
